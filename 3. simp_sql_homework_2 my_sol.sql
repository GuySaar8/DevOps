...
# was not able to do get a dynamic month hired
# was only able to run this command becuase we got info from Q1 and setting a spesific month
...
# was not sure how to make the JOB_ID dynamic
set @test = (select min(salary) from employees where JOB_ID="SH_CLERK");
select EMPLOYEE_ID,FIRST_NAME,LAST_NAME,SALARY 
from  employees
where salary > @test
order by salary desc;
...
# the only date in the table is hired date
# all employee were hired on the same year so I checked the amount paid by month
select EMPLOYEE_ID,sum(salary) as paid_months_7to9
from employees
